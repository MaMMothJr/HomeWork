...
switch (x) {
    ...
    case NaN: error = 'Не математическая операция'; break; //не нашел использования NaN в switch, я так понял так как NaN никогда не равне NaN то соотвественно нет возможности в этот кейс войти. При привидение переменной к числу, если это невозможно то дает NaN тут у меня ступор.
    default:
}

if (x == 0 || x == 1 || isNaN(x)) { //неправильно было заданно условие. // TODO: по идеи если x это NaN то условие должно выдать алерт эррором, но я не смог.
    alert(error);
} else {
    alert(100 / x);
